...
<div class="wow fadeInUp">
    <div class="panel">
        <div class="panel-heading">
            <h2 class="mt-none mb-none">OpenJobs</h2>
            <div class="panel-actions">
                <button onclick="location.href = '{{url('sales/openjobscopello/0')}}'" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> New Job</button>
            </div>
        </div>
                    ...
                    <tr class="gradeX">
                        @foreach($map as $label=>$col)
                            ...
                            @elseif($label=='Open')
                                <td>{{$data->$col?'Y':'N'}}</td>
                            @elseif($label=='Date')
                                <td>{{false===strpos($data->$col,'0000')?LT2IT($data->$col):''}}</td>
                            @else
                                <td>{{$data->$col}}</td>
                            @endif
                        @endforeach
                        <td><a href="{{url('sales/openjobscopello/'.$data->JobNRWebsite)}}" class="btn btn-primary btn-sm">Edit</a></td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
